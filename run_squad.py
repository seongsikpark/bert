...
flags.DEFINE_string(
    ...
    "predictions.json",
    "file name of predictions"
)

flags.DEFINE_string(
    "output_nbest_file",
    "nbest_predictions.json",
    "file name of nbest_predictions"
)

flags.DEFINE_string(
    "output_null_log_odds",
    "null_odds.json",
    "file name of null_log_odds"
)

flags.DEFINE_integer(
    "num_append_paragraphs",
    0,
    ...
    for entry in input_data:
        ...
        for (p_idx,paragraph) in enumerate(entry["paragraphs"]):
            ...
            if FLAGS.append_mode=='PRE':
                ...
                end_index_append=min(len(entry['paragraphs']),p_idx+FLAGS.num_append_paragraphs+1)
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            if FLAGS.verbose==True and example_index < 1:
                ...
                tf.logging.info("token_to_orig_map: %s" % " ".join(
                    ["%d:%d" % (x, y) for (x, y) in
                     six.iteritems(token_to_orig_map)]))
        ...
        #tf.logging.info("tokens_all: %s" % " ".join([tokenization.printable_text(x) for x in example.doc_tokens]))
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode,
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            #tf.logging.info('********** output_spec **********')
            #print(output_spec)
            #print(output_spec.predictions)
            #print(predictions['unique_ids'])
            #print(predictions['start_logits'])
        else:
            raise ValueError(
...
def input_fn_builder(input_file, seq_length, is_training, drop_remainder):
    ...
    #
    tf.logging.info('********** input_fn_builder **********')
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    #unique_id_to_features = collections.defaultdict(list)
    ...
    for result in all_results:
        ...
        #print(unique_id_to_feature[result.unique_id].start_position)
    ...
    #for (example_index, example) in enumerate(all_examples):
        ...
        #for (feature_index, feature) in enumerate(features):
            ...
            #print(_get_best_indexes(result.start_logits, n_best_size))
        #print(start_logits)
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        if example_index % 1000 == 0:
            tf.logging.info("Writing examples: %d" % example_index)
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            if not start_logits:
                start_logits=result.start_logits[:-1]
            else:
                start_logits+=result.start_logits[len(example.question_text)+2:]

            if not end_logits:
                end_logits=result.end_logits[:-1]
            else:
                end_logits+=result.end_logits[len(example.question_text)+2:]


            #print(len(start_logits))
            #print(len(example.question_text))
...
#                        end_logit=result.end_logits[end_index]))
        ...
        #print(prelim_predictions[0].start_index)
        #print(prelim_predictions[0].end_index)
...
def main(_):
    with tf.contrib.tfprof.ProfileContext('./profile/infer_test',trace_steps=range(0,100),dump_steps=[100]) as pctx:
        ...
        if FLAGS.do_predict:
            ...
            if not FLAGS.num_eval_examples==-1:
                eval_examples = eval_examples[0:FLAGS.num_eval_examples]

            print('********** num of eval examples **********')
            print(len(eval_examples))

            #tf.logging.info("********** print eval_examples **********")
            #print(eval_examples)
            ...
            for result in estimator.predict(
                ...
                if len(all_results) % 1000 == 0:
                    tf.logging.info("Processing examples (features): %d" % (len(all_results)))
            ...
            output_prediction_file = os.path.join(FLAGS.output_dir,FLAGS.output_prediction_file)
            output_nbest_file = os.path.join(FLAGS.output_dir,FLAGS.output_nbest_file)
            output_null_log_odds_file = os.path.join(FLAGS.output_dir,FLAGS.output_null_log_odds)

            #print(all_results[0][1])
            #print(len(start_logits))
            #
            #with tf.Session() as sess:
            #    print(sess.run(tf.argmax(start_logits)))
            #    print(sess.run(tf.argmax(end_logits)))
            #    sess.close()
            ...
            #with tf.contrib.tfprof.ProfileContext('./profile/infer_test',trace_steps=range(0,100),dump_steps=[100]) as pctx:
            #with tf.contrib.tfprof.ProfileContext('./profile/infer_gpu',trace_steps=range(0,1),dump_steps=[1]) as pctx:
            #with tf.contrib.tfprof.ProfileContext('./profile/infer_test') as pctx:
        ...
        print('write stat')
        print('num_append_paragraphs: %d'%(FLAGS.num_append_paragraphs))
        print('num_examples: %d'%(len(eval_examples)))
        print('num_features: %d'%(len(all_results)))
